...
const TransactionDetails = ()=>{
    ...
    fetch("https://finease-server-phi.vercel.app/FinEase")
      ...
      .then((allData) => {
        const sameCategory = allData.filter(
          (item) => item.category === model.category && item.type === model.type
        )
  ...
  return (
   ...
   <p className="text-lg font-semibold text-gray-700"> Total Amount of this Category ({model.category}) </p>

   <p className={`text-2xl font-bold ${ model.type?.toLowerCase() === "income"
           ? "text-green-600"
           : "text-red-600" }`} > {categoryTotal} </p>
       </div>
    </div>
      </div>
      </div>
